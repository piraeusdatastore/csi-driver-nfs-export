...
import (
	"github.com/kubernetes-csi/csi-driver-nfs-export-export-export-export/pkg/nfs-export-export-export-export"
	"github.com/kubernetes-csi/csi-driver-nfs-export-export-export-export/test/e2e/driver"
...
type DynamicallyProvisionedReclaimPolicyTest struct {
	...
	ControllerServer       nfs-export-export-export-export.ControllerServer
...
func (t *DynamicallyProvisionedReclaimPolicyTest) Run(client clientset.Interface, namespace *v1.Namespace) {
	for _, volume := range t.Volumes {
		...
		if tpvc.ReclaimPolicy() == v1.PersistentVolumeReclaimRetain {
			...
			// The controller server cannot resolve the nfs-export-export-export-export server hosting inside the testing k8s cluster, skipping the cleanup step.
