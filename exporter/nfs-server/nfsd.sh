...
# Check if the HOSTS_ALLOW variable is empty
if [ -z "${HOSTS_ALLOW}" ]; then
  echo "The HOSTS_ALLOW environment variable is unset or null."
  echo "This means any client can mount."
  > /etc/hosts.allow
  > /etc/hosts.deny
else
  echo "The HOSTS_ALLOW environment variable is set."
  echo "The allowed hosts are: ${HOSTS_ALLOW}."
  /bin/sed -i "s#{{HOSTS_ALLOW}}#"${HOSTS_ALLOW}"#" /etc/hosts.allow
fi
