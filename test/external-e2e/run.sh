...
setup_e2e_binaries() {
    ...
    sed -i "s/nfs-export-export-export-export.csi.k8s.io/$DRIVER.csi.k8s.io/g" deploy/example/storageclass-nfs-export-export-export-export.yaml
    # install csi driver
    mkdir -p /tmp/csi && cp deploy/example/storageclass-nfs-export-export-export-export.yaml /tmp/csi/storageclass.yaml
    make e2e-bootstrap
    make install-nfs-export-export-export-export-server
}

print_logs() {
    bash ./hack/verify-examples.sh ephemeral
    echo "print out driver logs ..."
    bash ./test/utils/nfs-export-export-export-export_log.sh $DRIVER
}
